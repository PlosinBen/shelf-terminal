 ...
 * recall (should NOT know 42). Each execution must idle; the session must survive
 * both slashes.
 *
...
rl.on('line', (line) => {
  ...
  if (m.type === 'message' && (m.msgType === 'reply' || m.msgType === 'fold_markdown'))
    console.log(`  [${m.executionId}] ${m.msgType}${m.label ? '(' + m.label + ')' : ''}: ${m.content ?? JSON.stringify(m.body ?? m.errorMessage ?? '')}`);
...
const replyOf = (tid) => lines.filter((m) => m.type === 'message' && m.msgType === 'reply' && m.executionId === tid).map((m) => m.content).join(' ');
...
async function execution(tid, prompt) {
  console.log(`\n— ${tid}: ${JSON.stringify(prompt)}`);
  send({ type: 'send', executionId: tid, prompt, ...base });
...
async function main() {
  ...
  await execution('e-1', 'Remember the number 42. Reply with just "ok".');
  check('e-1 idled', !!lines.find(idle('e-1')));

  const compactIdled = await execution('e-2', '/compact');
  ...
  await execution('e-3', 'What number did I ask you to remember? Reply with just the number.');
  check('context survives /compact (knows 42)', /42/.test(replyOf('e-3')));

  const clearIdled = await execution('e-4', '/clear');
  check('/clear idled + session alive', clearIdled);

  await execution('e-5', 'What number did I ask you to remember? If you do not know, say "UNKNOWN".');
  const r5 = replyOf('e-5');
