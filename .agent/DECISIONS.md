...
- Slash 輸出走 `slash_response` AgentMessage variant（pending → success/error upsert，同 msgId）
- Backend interface 只剩 `query(input, send)`，沒有 `handleSlashCommand`

跟 tool_use（#52）同哲學：renderer 給框、provider 給內容。Lifecycle 對齊：slash 在外部就是個 turn，streaming → idle，跟 `queuedMessages` queue 邏輯共用、不需插隊。
...
InputZone ──emit('agent:send', ...)──▶ EventBus
                                        ↓
                              App.tsx handler:
                              - agentMessageStore.appendUser(tabId, ...)
                              - shelfApi.agent.send(tabId, ...)
                                        ↓
                              agentMessageStore (per-tab)
                                        ↓ subscribe
...
- **App.tsx handler** — 統一處理 IPC + 寫 store（對位 Decision #1）
- **agentMessageStore** — per-tab scoped，唯一 message state

兩個 sibling 間 **間接相依**：input 送出的訊息透過 store 流到 MessageList，沒有 prop 直接串、沒有共享 state 持有者。

**原因**:

1. **對位 Decision #1**：CLAUDE.md 規範「Trigger sites only emit events, side effects in App.tsx」。AgentView 1186 行內的 `handleSend` 同時做 setMessages / setQueuedMessages / setFollow / IPC.send — 直接違反此規範
2. **解開 1186 行 god component**：當前 AgentView 同時 own messages state、input value、slash menu state、permission/picker UI、status bar、auth screen、plan panel。任何子功能改動都會擴大 re-render scope（agent-view-perf.md 的根因）
3. **正確架構讓 perf 問題自然消失**：MessageList subscribe store slice → 只在 messages 變化時 re-render，input 打字不再波及。**不需要手動 memoize 一堆 props**
4. **Tab unmount 才安全**（agent-view-perf.md Step 4）：messages 在 store 不在 component state，non-active agent tab 可以 unmount 釋放記憶體不掉資料

**為什麼是這個架構而不是其他**:

| 選項 | 為什麼不採用 |
|------|------------|
| 父層 coordinator + 共享 state | 跟現在 AgentView 一樣，只是換包裝 |
...
「先架構、後 perf」— 在錯誤架構上 memoize 會堆 useCallback / useMemo / props 套件，重構時全部要拆，白工。順序：

1. **新增 `agentMessageStore`**（per-tab slice：messages / status / streaming / picker / permission）
2. **App.tsx 增 event handler**（`agent:send` / `agent:stop` / `agent:clearHistory`）+ IPC listener 寫 store
3. **拆 `<MessageList tabId={...} />`**（subscribe store，不接 messages prop）
4. **拆 `<InputZone tabId={...} />`**（textarea + slash menu + ESC + attachments + chatStage useEffect）
5. **拆 `<StatusBar>` / `<DecisionPanel>` / `<PlanPanel>` / `<AuthPane>`**
6. **AgentView 變 layout coordinator**（lifecycle + composition，無 state）
7. **回頭做 perf 量測**：應該大部分自動好；殘餘問題（streaming batching / xterm scrollback）獨立修

**不要改**:

- **不要先做 memoize 再重構** — 重構時所有 memo props plumbing 都要拆掉重來
- **不要把 input 跟 MessageList 透過共同父層 state 串** — 父層 state 變化照樣 cascade re-render，沒解決問題
- **不要把 messages 留在 component state、其他搬 store** — Step 4 (tab unmount) 會掉資料
- **不要把 handleSend 留在 InputZone** — 違反 Decision #1，side effect 該在 App.tsx
- **不要為了 Phase 1 寫一份「過渡用 props 結構」** — 多寫一次又拆一次，直接走 store

**已知 trade-off**:

- 重構工程量大（拆 5-6 個 PR），短期內 perf 問題持續
- 期間使用者可以靠**換 project 暫時釋放記憶體**這個既有 workaround 撐
- 換得長期架構正確 + perf 自然解 + tab unmount 可實作
