...
## 45. Copilot API 模式（Chat Completions vs Responses）由 Fallback Cache 決定

**決策**: 不 hardcode model prefix，採 try-and-cache：
1. 預設用 `/chat/completions`
2. 收到 `unsupported_api_for_model` error → 透明 fallback `/responses`
3. 結果存記憶體 cache `Map<modelId, 'chat' | 'responses'>`
4. 同 session 內後續 query 直接走 cached api，無 retry cost

**原因**:
- Copilot `/models` API 不告訴我們該打哪個 endpoint，hardcode prefix（`gpt-5*`）會在 GitHub 加新 model 時錯判
- Empirical 比 prediction 準確 — 唯一可靠的是 server 自己回的錯誤
- First-query cost ~200ms，使用者無感（透明 retry，UI 沒看到失敗）
- Cache scope 是 backend lifetime，過期重啟會重 probe，但這是 acceptable trade-off（不需要 persist cache）

**不要改**: 不要回去用 hardcoded prefix list — 永遠追不上 Copilot 加新 model 的速度。不要把 cache persist 到 disk — 只是 session-local 優化，且 GitHub 改 model 行為時 stale cache 反而誤判。

---

## 46. Copilot Responses API 用 previousResponseId 串接，不 replay history

**決策**: 對 stateful Responses API（gpt-5 family 等），client 端**不**送 message history，而是：
- 第一輪：傳新 user message（不帶 `store` 或 `previousResponseId`），server 預設會記
- 第二輪起：傳新 user message + `previousResponseId: <上次的 id>`，server 自己接前面狀態
- 跨 turn 只在 client 保留 `lastResponseId`，`modelMessages` 維持空

**Copilot 特例**: OpenAI 標準 Responses API 用 `store: true` 控制 server 是否保留 state，但 Copilot 不支援這個 param（回 `unsupported_value`）— Copilot 似乎強制保留。我們不傳 `store`，靠 `previousResponseId` 串接即可。

**原因**:
- Replay 整段 history 包含 tool_call IDs，server 認不得（client 重新 generate 的 ID 跟 server 記的不一樣）→ 回 404 not_found
- Responses API 設計就是 stateful，跟 Chat Completions（stateless replay）是不同 paradigm
- 跟 Claude SDK 一樣 hands-off：context 管理外包給 server，client 不算 token、不 compact、不替 server 操心
- Persist 也只存 `lastResponseId`，省空間

**配套**:
- Auto-compaction 在 Responses 模式下停用（server 自管）
- `/compact` 在 Responses 模式回 system message 提示 user 用 `/clear` reset
- `/clear` 把 `lastResponseId` 設 null，下一輪重新從 fresh state 開始

**不要改**:
- 不要在 Responses 模式 replay modelMessages — 一定 404
- 不要 client 端硬塞 tool_call id 想跟 server 對齊 — id 是 server 生成的，無法預測
- 不要在 Responses 模式跑 client-side compaction — server 自己會處理，重複壓縮浪費 token
