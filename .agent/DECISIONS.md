...
**命名歷史**: 早期叫 `plan_update`，2026-05 配合 canonical msg type 重構（見 `.agent/features/AGENT_VIEW_MSG_TYPE.md`）改名為 `plan` —— union 內其他 type 都是名詞，rename 對齊。
...
詳細討論軌跡見 `.agent/features/AGENT_VIEW_MSG_TYPE.md`「設計選擇 3」。
...
- Lifecycle events（`ready` / `pong` / `capabilities` / `credential_*` / `slash_result`）在 turn 外部，turnId 是 optional — 由 requestId 或單一 dispatcher 處理
...
詳細討論軌跡見 `.agent/features/AGENT_VIEW_MSG_TYPE.md` 的「第二輪：P1 + P2 架構級重構」section。
