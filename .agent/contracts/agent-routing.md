...
- Copilot first establishes the final session: `session/resume` when `sessionCapabilities.resume` is advertised, otherwise `session/load` when `loadSession` is true; load replay content is suppressed while metadata is retained. A capabilities-time `session/new` emits `context_patch` immediately. It then hydrates the provider snapshot, validates all saved selections, and applies differing values sequentially as model → effort → native mode → native permission. Intermediate provider updates do not publish capabilities. The RPC resolves only with the final confirmed snapshot; validation, setter, mismatch, timeout, or teardown rejects initialization instead.
