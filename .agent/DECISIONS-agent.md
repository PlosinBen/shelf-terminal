...
## 45. Copilot 依賴 CLI 自己的登入狀態，token 完全不經手，且不綁 `gh`

**決策**: `CopilotClient` 啟動時用 `useLoggedInUser: true`，**不**傳 `gitHubToken` → 直接吃 Copilot CLI 自己的 OAuth / device-flow 登入（存在 CLI 自己的 config，若有 gh 登入也會被 SDK 採用，但 `gh` 指令本身**不是依賴**）。沒登入 → throw 提示登入 Copilot。

**原因**:
- 跟 Claude 一致：不經手 token，依賴使用者本機官方 CLI 的登入狀態（Decision #43 provider 抽象原則）。
- 不綁 `gh`：多裝一個工具違反零依賴定位；且遠端執行時 `gh auth token` 會在遠端跑，本機拿的 token 無意義。
- 傳 `gitHubToken` 會強制 `useLoggedInUser` off 並要求外部 token source（如 gh）→ 刻意不傳。

**不要改**:
- 不要傳 `gitHubToken` / 不要回去跑 `gh auth token` — 會把 `gh` 變成硬依賴，且遠端語意錯。
- 不要自己存 token 到 userData — keychain ACL 是 per-binary 綁定，自存等於把 GitHub OAuth refresh 邏輯重做一遍。

> 歷史：早期版本曾用 `useLoggedInUser: false` + `gh auth token` + `gitHubToken` 覆寫（為了繞過 macOS keychain 提示）。後來改回 `useLoggedInUser: true` 以拔掉 `gh` 依賴。
