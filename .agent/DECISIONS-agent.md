...
InputZone parseSlashPrefix
    ...
    │     → 開 inline <SelectionPanel>（renderer-only，不送 IPC）
    │       picker 選定 → handleConfigEdit
    │       (status bar 立即反應；長期可收斂到 provider 路徑)
    │
...
- 有 args 走 provider slash（如上圖），provider 自己呼 imperative setX 後 re-broadcast capabilities
