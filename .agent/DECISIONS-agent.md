...
**待辦（未做）**：~~真機驗 Claude 載入 skills~~ ✅ 已驗（見上）；**Copilot 載入 skills 仍待真機驗**（無 session 可測）；user-invoke 一鍵觸發（`/<name>`，兩家不對稱）；degraded 健康狀態的 E2E（需 fake provider 支援延遲/丟 pong）；健康顏色 per-theme severity token 化（目前 hex）。
