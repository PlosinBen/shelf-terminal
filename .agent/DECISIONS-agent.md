...
## 64. Provider model/config 職責邊界：不做多餘驗證，SDK 唯一仲裁

**背景**：曾為修「Copilot 卡在外來 model id（`claude-opus-4.8` 漏進 `agentPrefs.copilot`）每回合報 not available」而在 provider 加自訂驗證（比對 `listModels()` 擋未知 id）。判定為**擴張 provider 職責**後撤回。把 #55「不要在 renderer 加 model validation」的原則明確延伸到 **provider 端**。

**決策**：
- **Provider 不自己驗證 model**。除非 SDK 真的有 validate API，否則不前置擋。合法集 = picker list（`capabilities.models` ← SDK `listModels()`），picker 值本來就由 SDK 決定。
- **唯一權威是 SDK runtime**：直接 `session.setModel()`（Copilot 有 session 時）或下次 `query()`；成功就用，**失敗照實 emit error**。不維護自家白名單前置拒絕——清單會落後 GitHub 實際支援（例：opus 4.8 已上線但 `listModels` 還沒列，前置擋會誤殺）。
- 「卡在外來 id」的復原靠 **SDK 報錯 + 使用者改選有效值**（picker 只列合法值），不是自癒驗證。

**Renderer / Backend 分層（回應「picker 兩邊行為是否不同」— 不同的只在 backend）**：
...
- **差異只在 backend apply 語意（本質差異，勿為對稱強行統一）**：
  - Claude：lazy options → `applyConfigEdit`（純 set+emit，無 SDK 呼叫，當下不會錯）。#63 講的收斂點。
  - Copilot：eager SDK 呼叫 → `query()` 必須把 `QueryInput.configEdit` 路由進 `dispatchSlash`（`permissionMode`→`/permission`）。**漏了會 fall through 成空 prompt 送出 → 沒卡片、接續上次對話**（曾經的 bug）。
- 兩邊 config-edit 成功都 emit `system` divider（共用 `formatConfigAck`，文案「applies on next query」對兩邊都成立）；Copilot 失敗 emit `error`。`/help`/`/clear`/`/context`/`/compact` 仍是 `fold_markdown`（slash 內容輸出，非狀態轉換）。

**不要改**：
- 不要在 `gatherCapabilities`/`setModel`/`dispatchSlash` 加「model 是否在清單內」前置拒絕 — 交給 SDK，錯誤照實回。
...
**Related**：#55（slash routing + prefs flow）、#63（config-edit 收斂）、#62（model 顯示 intent-driven）、`agent-server/providers/{claude,copilot}/index.ts`、`src/shared/config-ack.ts`
