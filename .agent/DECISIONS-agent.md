...
**驗證**:store tombstone 防復活 + `/clear` 重置(單元,`agentTabStore.test.ts`);running `×` → stopTask 經 SDK → 確認後移除(E2E,`agent-background-tasks.spec.ts`,fake backend `stopTask` emit `stopped`)。
...
**修正**:前景 turn 內被壓住的 task 事件累積到 `pendingForegroundTaskEvents`,`closeForegroundTurn` **先逐一 flush(含 terminal 事件)再發 running snapshot**。start/cancel 會清空累積。如此完成於 turn 內的任務也會以 `done` 卡送達;snapshot 仍負責跨 turn 的 still-running 對帳。
...
這些正常情況永不觸發;一旦出現在 agent-server stderr / devtools console,即坐實某類 wire shape 沒被處理。**已知 by-design 的略過**(routeServer 只處理 auto-resume turn 的 assistant、ambient task 隱藏)維持靜默,屬刻意行為。
