...
**清理一律走 TTL / last-touch sweep，不 eager 互刪**：
- ⚠️ **既有坑（待修）**：`remote.ts:454` agent-server 版本清理 = 「刪掉除當前版本外所有版本」，**隱含「一台 server 一個 app」**。m:n 反例：共享 server 上 app A(v2) 部署會刪掉 app B(v1) 在用的 v1 → B 下次 spawn 重部署 v1 又刪 v2 → **互刪 + 反覆重傳 215MB（thrash）**。
- **正解 = 改清理策略**（非改 key）：每次連線（即使 skip deploy）`touch` 該版本 sentinel；啟動時 sweep 掉 mtime 超過 N 天者。同時保住 **dedup + m:n 安全 + 自清**。per-app（apps/<appId>）的清理同理走 TTL（沿用 `context-store.cleanupOldContexts` 模式）。
...
**Related**：#32（agent-server bundle deploy + `.deployed`）、#35/#38（context 持久化 / `os.homedir()` 定址）、#43（provider 差異封裝）、`src/main/agent/{remote.ts,deploy-layout.ts}`、`agent-server/context-store.ts`（TTL sweep 範本）。**skills feature 是此規則第一個 per-app 投影實例**（規劃中，實作後補述消費點接線）。
