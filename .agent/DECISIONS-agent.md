...
- **stop-task**（從面板中止背景任務）：需 Architecture B（persistent streaming-input session，SDK 正規背景模型才有 `stopTask`）—— 比現行 detached-loop 大，等真有需求再做。
...
- **消費端統一走「投影 + 兩家各自指」**（#70 的第一個實例）：投影到 `os.homedir()/.shelf/apps/<appId>/skills`（local fs cp = `skills-projection.ts`；remote scp/docker cp/wsl = `remote.ts syncSkillsToRemote`，content-hash `.synced` gate）。Claude `options.plugins=[{type:'local',path:<root>}]`（自訂目錄的唯一官方解法，per-query 故編輯下一輪生效，skill 顯示為 `shelf-skills:<name>`）；Copilot `createSession({skillDirectories:[<root>/skills]})`（session-cached，新 skill 可能需 reopen/`/skills reload`）。`providers/shared.ts resolveSkillsPluginRoot` 路徑存在才指。**renderer 對此無感**（守 #43）。
...
**待辦（未做）**：真機驗 Claude/Copilot 實際載入 skills；user-invoke 一鍵觸發（`/<name>`，兩家不對稱）；degraded 健康狀態的 E2E（需 fake provider 支援延遲/丟 pong）；健康顏色 per-theme severity token 化（目前 hex）。
...
- **turn 對應（純函式 `claude/turn-router.ts`，9 單測）**：result 無 backref，靠順序 + wire 信號。每個 turn（前景或 auto-resume）開頭都有 `system/init`；auto-resume **必由前一個 `task_notification` arm**；前景 result 無 `origin`、auto-resume result 帶 `origin.kind==='task-notification'`（spike full-dump 確認 init/assistant 兩者無法自我區分）。狀態機：`task_notification`→arm；`init`→armed 開 server turn 否則 consume FIFO front 開前景 turn；assistant/stream/tool→active lane；`result`→收對應 lane（origin 驗證）。
...
- **控制方法**：`setModel`/`setPermissionMode` → SDK control method（mid-session 即時，免 re-resume）；**effort 無 control method → close 舊 query + `resume=lastSessionId` 重建**（罕見、最 robust）；`stop()` → `interrupt()`（中斷當前 turn、**session 存活**）；`dispose()` → `close()`。
...
**待辦（未做，低優先）**：FIFO 邊角（task settle 與排隊新 turn 的 init 競爭、counter 可能誤配，result.origin 可事後偵測）；`/clear`·`/compact` 在 streaming 模式的 context reset 行為真機驗；持久 query 崩潰恢復（resume 重建 + 收尾在途 turn，目前 `teardownTurns` 發 idle）；`stopTask()`/`backgroundTasks()` 主動控制（SDK 已有，未接 UI）。

**Related**：#69（被取代的 detached-loop；task_event/server-turn 渲染仍沿用）、#43（provider 封裝）、`agent-server/providers/claude/{index,turn-router,turn-router.test}.ts`、`agent-server/providers/claude/background-tasks.test.ts`、`scripts/{spike,smoke}-streaming-input.*`。
