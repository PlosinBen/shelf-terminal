...
**解法**: `case 'assistant'` 每次收到 top-level（非 subagent）的 assistant message 時，把 `msg.message.usage` 存進 `lastTurnUsage`。`case 'result'` 優先用 `lastTurnUsage`（per-turn），fallback 才用 `msg.usage`（可能 cumulative）。分子 = `input_tokens + cache_read_input_tokens + cache_creation_input_tokens`；分母仍取 `modelUsage[currentModel].contextWindow`。

**注意**:
- `result.usage` 實測在多 tool-use turn 的 query 中是 cumulative（加總所有 turn 的 input），造成 147% 而 `/context` 只報 49%。debug log（`result.usage` vs `result.lastTurnUsage`）可確認。
- OpenAI-style engine（Copilot / Gemini）的 `usage.prompt_tokens` 本來就是 per-turn 總量（cached tokens 是它的子細項），不用拆分。

---

## 32. Claude 訊息流混有 subagent — 廣播 UI 狀態要濾掉 `parent_tool_use_id`

**現象**: Status bar 的 model 在 Opus / Haiku 之間亂跳——同一個 user turn 內先變 Haiku、又變回 Opus。

**原因**: Claude Code 內部會起 Haiku subagent 跑側邊任務（tool summarisation、plan-mode 分析、背景摘要等）。這些 subagent 會吐自己的 `SDKAssistantMessage`，欄位 `message.model` 是 `haiku-...`。`case 'assistant'` 無條件把這個 model 塞進 status event → renderer 的狀態列就跟著變。

**判別**: subagent 的 assistant message 一定會帶 `msg.parent_tool_use_id`（指向觸發它的 tool use）。top-level 主模型回覆這欄位是 `null` / `undefined`。

**解法**: `src/main/agent/providers/claude.ts` 的 `case 'assistant'` 裡，只有 `parent_tool_use_id == null` 才把 `model` 塞進 status payload。token 計數等其他欄位可以照常帶（使用量本來就該合計，雖然 UI 目前沒分開顯示）。

**注意**: 未來若要顯示「現在在跑 subagent」，走新欄位（例如 `status.subagent = { model, parentToolUseId }`）而不是重用 `model` 欄位——語意要清楚，避免下一個人又把它拿去覆蓋狀態列。

---

## 33. Agent 訊息提交走 event bus，不要在 trigger site 做 side effect

**現象**: 使用者在 agent 還在跑的時候再打一則訊息進 queue，上一輪結束後自動接上，結果訊息有送到 backend、model 也回了，但訊息本身在 transcript **沒有顯示**。

**原因**: 「送 user message」在語意上是一個動作，但實作拆成三步：(1) `addAgentMessage()` 進 in-memory transcript，(2) `saveMessage()` 寫 IndexedDB，(3) `window.shelfApi.agent.send()` 送 IPC。早期 AgentView 的 Enter 路徑三步都做對；App.tsx queue flush 卻只做了第 3 步——queued message 因此永遠不進 UI。

**解法**: 統一走 `Events.AGENT_SUBMIT_MESSAGE`。trigger site（Enter、queue flush、未來的 retry button）只負責 `emit(payload)`；`submitAgentMessage()`（in `src/renderer/agent-actions.ts`）是唯一的 side-effect handler，註冊在 App.tsx。這也呼應 CLAUDE.md 的 convention：「All user actions go through event bus」。

**注意**: queue 的 storage schema（`{ id, content }`）目前不帶 attachments——使用者在 streaming 中選的檔案 / 貼的圖片會被捨棄。fix 有另外議題，修的時候記得把 attachments 納入 queue entry 並傳進 payload。
