...
修法(`1248d29`):`flushArmedRef` guard —— flush 時 disarm、`isStreaming` 真正變 true 時才 re-arm,確保每個 streaming→idle 週期只 flush 一條。因為每條被送的訊息(含 instant config-edit)都保證會發 `streaming`,re-arm 有保證、不會 stall。**別把 flush 改回純 level-trigger**,也別假設送出後 `isStreaming` 已是 true。迴歸測試:`e2e/agent-flows.spec.ts`「flush one at a time」(斷言 queue `2→1→0`)。
