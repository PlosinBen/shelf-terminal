...
| App 層 Agent Skills（CRUD） | `skills-store.ts` | `<userData>/skills/` 下的 app 層 skill 管理（layout = Claude plugin root：`.claude-plugin/plugin.json` + `skills/<name>/SKILL.md`）。SKILL.md 視為使用者 opaque raw md（只 parse name/description 供列表）；**frontmatter `name` = identity**，存檔時 folder 跟著 rename（kebab 驗證 + 撞名檢查 + 整包搬移）。見 DECISIONS-agent #71 |
...
| 連線健康（heartbeat RTT） | `connection-health.ts` | `ConnectionHealthTracker` 純狀態機（clock 注入）：app↔agent-server 心跳（`ping`/`pong` 帶 seq）的 client-side RTT → `healthy/slow/unstable/dead`。RTT 全用 client 單邊時鐘（pong echo seq，server 時鐘不進比較）。`remote.ts wrapProcess` 跑心跳 timer + 攔 pong + `onHealth`（→ `AGENT_CONNECTION_HEALTH` IPC）。見 DECISIONS-agent #71 |
...
| Turn 路由（claude）| `providers/claude/turn-router.ts` | 純 attribution 狀態機（Architecture B / DECISIONS #72）：streaming-input 持久 session 一條 generator 跑多 turn，SDK 無 message→user backref，故靠順序 + wire 信號把每則訊息分到 foreground / server / task lane。`routeMessage(state, {type, systemSubtype, resultOrigin})` → `{lane, start?, close?}`；`task_notification` arm 下一個 `init` 為 auto-resume server turn。`turn-router.test.ts`（9 case，用真 spike 序列驗）。**只被 `claude/index.ts` 引用** |
