...
| App 層 Agent Skills（CRUD + lock） | `skills-store.ts` | `<userData>/skills/` 下的 app 層 skill 管理（layout = Claude plugin root：`.claude-plugin/plugin.json` + `skills/<name>/SKILL.md`）。SKILL.md 視為使用者 opaque raw md（只 parse name/description 供列表）；**frontmatter `name` = identity**，存檔時 folder 跟著 rename（kebab 驗證 + 撞名檢查 + 整包搬移）。**`updateSkill` 是 upsert**（create flow 依賴它：placeholder dir → updateSkill 寫內容）。**Lock**：folder 內 `.locked` marker（`isSkillLocked`/`setSkillLocked`，in-folder → rename 自動帶走），`listSkills` 回 `locked`。見 DECISIONS-agent #71/#77 |
