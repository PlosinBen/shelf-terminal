...
| Agent 對話 UI | `components/AgentView.tsx` + `components/agent/MessageList.tsx` + `agentTabStore.ts` + `agentTabSubscriptions.ts` + `agent-message-builder.ts` | 重構後架構（Decision #59 / `features/agent-view-refactor-plan.md`）：AgentView 是 layout host + 餘下 input 處理（PR 5 會再拆 InputZone）。Domain state（messages / status / capabilities / decisions / auth / init / queue / plan）在 `agentTabStore`（per-tab listener，避免 store.ts 的全域 snapshot 重建）。`bindAgentIPCGroup` 在 App.tsx mount 時把 IPC 接到 typed bus；`bindAgentStoreSubscriptions` 訂閱 bus → 寫 store。`MessageList` 自訂閱 store 顯示 timeline，跟 input 解耦（perf 修復根因）。**Slash 分兩類**：agent-bound（/help /context /compact /clear）走 emit 'agent:send' → provider dispatchSlash；renderer-local（`RENDERER_LOCAL_SLASHES`：/model 目前；/effort /permissionMode 預留）由 `handleConfigEdit` 直接 setActualX + persistPref，零 IPC，下次 send 自然帶上新 pref。**Prefs (model/effort/permissionMode) 是 renderer-authoritative** — projectConfig.agentPrefs 是 intent source of truth；store.actual* 是 backend reported display。Capabilities event 不 fall back 到 intent（修復 reconnect 時 actual 被 intent 蓋掉的 latent bug） |
