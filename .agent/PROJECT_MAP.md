...
| Skills 變更後處理（統一 pipeline） | `skills-sync.ts` | `onSkillsChanged()`：**任何** skill mutation 後的單一出口（觸發點 = manager UI 的 `ipc/skills.ts` 或 agent bridge 的 `app-tool.ts`，兩者都呼叫它）。依序：①本機 re-project（`projectSkillsLocal`）②跑 subscribers（由 `agent/index.ts` 經 `subscribeSkillsChanged` 注入，避免 `remote→app-tool→skills-sync` cycle；做 remote re-mirror **＋ live hot-reload**——檔案落地後對每個 live session 打 `reloadSkills`，local 立即／remote 先 sync 再 reload，skill 改完免重連即生效，見 #80）③通知 renderer（`SKILLS_CHANGED` → SkillsView 刷新）。見 DECISIONS-agent #77/#80 |
