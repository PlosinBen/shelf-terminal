...
| PTY spawn/kill/resize | `pty-manager.ts` | node-pty 管理、local/SSH/WSL/Docker spawn 方式、idle notification |
...
| 檔案上傳（paste / drag-drop）| `file-transfer.ts` | `uploadFile(connection, cwd, filename, buffer)`，所有 transport 都用 `cat`-via-stdin 寫到 `<cwd>/.tmp/shelf/` |
...
| Connector 測試 | `connector/ssh.spec.ts` | SSH connect/multiplex/file upload（需 docker test container） |
| Connector 測試 | `connector/docker.spec.ts` | Docker exec spawn 與 file upload（需 docker test container） |
| 單元測試 | `src/main/updater-state.test.ts` | Updater reducer 21 個 transition 測試 |
| 單元測試 | `src/main/file-transfer.test.ts` | filename / prefix / shell-quote / path build 純函式測試 |
