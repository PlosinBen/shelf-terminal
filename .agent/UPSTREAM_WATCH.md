...
- [GitHub Copilot CLI ACP server](https://docs.github.com/en/copilot/reference/copilot-cli-reference/acp-server) — bundled Copilot ACP 會在新 prompt 前 abort 上一輪，並把 cancellation `session.info` 壓成 assistant text。TRACKER: no exact public tracker。STANCE: Copilot provider 窄過濾精確的 `Info: Operation cancelled by user` adapter 產物。WHEN FIXED: 升級 pinned Copilot CLI，實測連續 prompt 不再產生假 user-cancel chunk或 ACP 保留 typed info，再移除 filter、常數與 regression test。 [→ context/agent-providers#48]
