...
- [electron-userland/electron-builder releases](https://github.com/electron-userland/electron-builder/releases) + [brace-expansion GHSA-mh99-v99m-4gvg](https://github.com/advisories/GHSA-mh99-v99m-4gvg) — 最新 stable builder 仍經舊 ASAR/universal/ejs 鏈帶入 vulnerable `brace-expansion`，完整 audit 因此留下 build-only high；runtime audit 為零。STATUS: **TRACKING**。TRACKER: no exact public tracker；回檢時先搜尋 builder issue / PR。STANCE: 保持最新 stable，不用會破壞 CommonJS API 的 v5 override，也不接受 `npm audit fix --force` 反向降版。WHEN AVAILABLE: 升到採用相容 patched brace-expansion dependency chain 的 stable builder、確認完整 audit 清零並移除此條。 [→ context/build-packaging#7]
