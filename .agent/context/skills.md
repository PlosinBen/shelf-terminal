...
**Decision C — 統一 mutation pipeline**（`skills-sync.ts onSkillsChanged()`）：所有觸發點（manager UI 的 `ipc/skills.ts`、agent bridge 的 `app-tool.ts`）mutation 後**只呼叫 `onSkillsChanged()`**：①本機 re-project ②跑 subscribers（remote re-mirror + hot-reload，由 `agent/index.ts` 經 `subscribeSkillsChanged` **反向注入**避免 `remote→app-tool→skills-sync` import cycle）③`SKILLS_CHANGED` 通知 renderer。觸發點只負責寫 store + 喊一聲。`onSkillsChanged()` 涵蓋的是**內容變更**（create/update/delete/aux）。**lock/unlock 不是內容變更**（skill bytes 不動）→ 本就不屬於這條 pipeline：`SKILLS_SET_LOCKED` 只呼叫抽出來的 `notifyRendererSkillsChanged()`（即 ③ 那步）重畫鎖頭 badge。lock 在 main 端強制、agent 從不讀,所以無需 re-project / re-sync / reload —— 若硬讓它走完整 `onSkillsChanged()`,反而會在無關的 live agent tab 冒出 `skills#9` 的 "Skills reloaded"。

**Decision D — per-skill lock（防全域污染）**：`update` 雖被 permission gate，但 **bypass/allow-all 會繞過** → lock 是不受權限模式影響的硬性「agent 別碰這顆」。folder 內 `.locked` marker（`isSkillLocked`/`setSkillLocked`；**in-folder → rename 自動帶走**），`app_skill.update` 在 **main 端**檢查 locked 就報錯（remote agent + bypass 都擋得住）。manager UI 永遠能改/解鎖；**agent 無解鎖工具**（同 delete 立場）。`.locked` 是 **main-only 控制檔**：`skills-projection` 的 `listSkillFilesRel` / `projectSkillsLocal` 都把它**排除**(basename `.locked`),所以它不外流到 consumption tree、也不進 `hashSkillsTree` → lock/unlock 讓投影樹與 hash **不變**(不觸發遠端 re-sync)。
