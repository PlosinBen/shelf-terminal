...
**Decision：** Copilot capabilities warm-up 將 project prefs 視為 initialization intent。它先 resume/load persisted ACP session；只有 `agent-providers#47` 的 exact missing-session error 才換成 new session。取得 final session 的 modes/config options 後，hydrate current model/effort，再完整驗證 saved model、effort、native mode、native permission。每個新建立的 final session 依序將存在的 saved selection 全部送進 SDK 一次，即使 restored value 已同值；完成後才回傳 final capabilities。重用已初始化 live session 的 capabilities read 不重跑 initial apply，後續真正不同的 intent 仍走一般 setter。Warm-up 期間 provider update 不對外發布 capabilities。

**Confirmation：** Model、effort 與 native permission 以 `session/set_config_option` 回傳的完整 config snapshot 為權威。Native mode 在呼叫 `session/set_mode` 前先註冊 waiter，完成條件是 RPC 成功且 observed mode 等於 requested mode；restored snapshot 已同值時 state predicate 已滿足，不要求 redundant notification，異值時接受同 session 的 `current_mode_update` 或 mode-bearing `config_option_update`。值不符、session/connection teardown 或 bounded timeout 都 reject。Waiter 必須先註冊，因為 notification 可能早於 set-mode response。

**Reason：** Copilot replacement session 會帶 provider defaults；若只修 session pointer，model/effort/native selections 仍會遺失。Restored snapshot 是 runtime status 的真相源，但不是 Shelf 已將 initialization intent 送進 final session 的證明。固定走 `final session → set perf → final capabilities` 可讓 successful resume 與 missing-session replacement 遵守同一份 project-level selection contract；抑制 intermediate capabilities 則避免 renderer 把 temporary defaults 當成 confirmed state 寫回 project prefs。

**Do not change casually because：** 不要在 initial apply 用 restored equality 省略 outbound setter，也不要把 initial apply 擴張成每次 capabilities read 或每個 turn 重送。Provider 只編排 SDK contract，不推測或複製 SDK cache/state-machine；只有 protocol-compliant apply 後仍確認是 SDK defect，才另立有 upstream trace 與移除條件的 workaround。不要在 `ensureSession` 前套 prefs、把 config acknowledgement 當 confirmed changed state，或遇到任一 warm-up failure仍回傳 partial capabilities。Fresh pre-session auth probe 可以回 `authRequired`；但 final session 已建立後的 validation/apply/confirmation failure 必須 fail initialization。
