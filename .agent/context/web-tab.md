...
- **`browser_fetch` 要跳過 provider 層提示**避免雙重提示。**claude**（SDK）`canUseTool` 對 `isWebFetchTool(toolName)` 直接 allow。**copilot 走 ACP 後**：shelf-bridge 工具（含 `browser_fetch`）由 L1 in-process HTTP MCP 暴露、經**通用 ACP permission bridge**（`acp/permission.ts`），**無 per-tool skip flag**（原 native SDK 的 `defineTool({skipPermission:true})` 已刪）→ ACP 路徑的雙重提示是否需抑制、如何抑制 = **待 field-test 驗證的 open item**。工具命名為 `browser_fetch` 而非 `web_fetch`:Claude SDK 內建了同名 `web_fetch`,外部工具撞名會直接報錯;且語意不同（內建 `web`=匿名抓公網、本工具 `browser`=帶你瀏覽器分頁的登入 session）。
...
- **沿用 `browser_fetch` 的自帶 gate 模式**（web-tab#2）：claude `canUseTool` 對 `isBrowserOpenTool` 直接 allow 跳過 tool 提示；copilot（ACP）的 `browser_open` 由 L1 HTTP MCP 暴露、走通用 ACP permission bridge（無 per-tool skip flag，原 native SDK `skipPermission:true` 已刪），ACP 路徑是否雙重提示待 field-test。真 gate 在 main `handleAppTool('web.open')` → `requestBrowserOpen`（`src/main/browser-open.ts`，自有 `web:browser-open-request/-resolve/-close` IPC，跟 agent timeline 脫鉤）。bypass 模式照樣 gate（tool 仍執行到 handleAppTool）。
