...
- **The renderer timeline has no turn boundary.** User, assistant, system, note, error and fold messages render linearly in store order. Never make message visibility, DOM grouping, spacing, or insertion depend on `turnId` or a derived start-of-turn marker.
