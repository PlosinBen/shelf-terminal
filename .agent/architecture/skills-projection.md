...
- Disabling a skill is the deliberate opposite of locking. Both are user-only markers held at the source and both markers are excluded from every projection — but disabling *does* enter this fan-out, because a disabled skill's whole folder is dropped from the derived copies. That drop changes the file list and the content hash, so the change-hash gate re-pushes and live sessions hot-reload the skill out of context (re-enabling reverses it). The distinction is the point: a lock must leave the projection invariant, a disable must perturb it.
