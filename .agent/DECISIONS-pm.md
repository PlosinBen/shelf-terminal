...
**Telegram-relayed prompt 加 channel hint（no-markdown + 壓長度）**: agent mode 下 `routeMessageToAgent` 把 `TELEGRAM_BRIEF_HINT` append 到送給 agent 的 prompt。Hint 處理**兩個同源限制**(都是「對面是 Telegram 手機」):(1) **純文字**——Telegram 這條不 render rich text,叫 agent 別輸出 Markdown(`**` `_` `#` `` ` `` code fence / table);(2) **短**——螢幕 + 4096 限制,多議題時**只列 topic 標題、讓使用者主動問哪個展開**,別一次全講;長 code/output 說「see Shelf」不要貼。Hint **只注入 agent context**——`sendFromInternal(tabId, prompt, displayText)` 第三參數讓 Shelf 的 user 泡泡 mirror **原文**,hint 不漏進 UI。理由:bridge 唯一能影響 agent 行為的槓桿就是 user-turn 文字(CLI agent 是獨立 process、碰不到它的 system prompt),等同 #41 PM 的 per-turn reminder。
