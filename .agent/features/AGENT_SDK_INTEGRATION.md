...
兩者都走 OpenAI-compatible API，各自只提供連線設定，底層共用 `openai-processor` 實作 agent loop。新增 OpenAI-compatible provider（Ollama、Mistral）只需寫 thin wrapper。
...
OpenAI 沒提供官方 API 查「哪個 model 吃 `reasoning_effort`」。`processor-tools.ts` 的 `getEffortLevels(modelId)` 用 pattern match 推論：
