...
interface ShelfApi {
  ...
  worktree: {
    ...
    onProposeCreate: (callback: (payload: { projectId: string; branch?: string; notePaths?: string[] }) => void) => () => void;
  ...
  configBackup: {
    ...
    listSources: (remoteUrl: string) => Promise<import('../shared/config-backup').BackupSource[]>;
    listImportItems: (
      remoteUrl: string,
      sourceRevision: string,
    ) => Promise<import('../shared/config-backup').ImportListResult>;
    applyImport: (
      remoteUrl: string,
      sourceRevision: string,
      selectedIds: string[],
  ...
  agent: {
    ...
    send: (tabId: string, prompt: string, images?: string[], prefs?: { model?: string; effort?: string; permissionMode?: string; attachments?: import('../shared/types').AgentAttachment[]; configEdit?: { key: import('../shared/config-ack').ConfigEditKey; value: string }; clientMsgId?: string }) => Promise<boolean>;
