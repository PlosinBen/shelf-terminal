...
import { AGENT_PROVIDERS, CODEX_PROVIDER, agentProviderEntries } from './agent-providers';
...
describe('AGENT_PROVIDERS registry', () => {
  it('enumerates exactly the known providers, in order', () => {
    ...
    expect(agentProviderEntries().map(([id]) => id)).toEqual(['claude', 'copilot', CODEX_PROVIDER]);
  });
