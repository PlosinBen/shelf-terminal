...
describe('detectTargetFromProbe', () => {
  ...
  it('REFUSES x64-musl (alpine x64) — Node has no official musl build', () => {
    expect(() => detectTargetFromProbe('x86_64\n/lib/ld-musl-x86_64.so.1')).toThrow(
      UnsupportedTargetError,
    );
  });
  it('REFUSES arm64-musl (alpine on ARM)', () => {
    expect(() => detectTargetFromProbe('aarch64\n/lib/ld-musl-aarch64.so.1')).toThrow(
      UnsupportedTargetError,
    );
  });
  it('musl error message mentions Alpine / no official musl build', () => {
    expect(() => detectTargetFromProbe('x86_64\n/lib/ld-musl-x86_64.so.1')).toThrow(/musl|Alpine/i);
  });
  it('ignores blank leading lines before uname output', () => {
    const t = detectTargetFromProbe('\n\nx86_64\n/lib64/ld-linux-x86-64.so.2');
    expect(targetId(t)).toBe('x64-glibc');
  });
  it('supported set is exactly the two glibc targets', () => {
    expect([...SUPPORTED_TARGETS].sort()).toEqual(['arm64-glibc', 'x64-glibc']);
  });
});
