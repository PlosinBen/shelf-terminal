...
interface IncomingMessage {
  type: 'send' | 'stop' | 'cancel_queued' | 'ping' | 'resolve_permission' | 'resolve_picker' | 'get_capabilities' | 'store_credential' | 'clear_credential' | 'clear_context' | 'read_task_output' | 'stop_task' | 'reload_skills' | 'app_tool_result' | 'teardown';
  provider?: Provider;
  /**
  ...
  taskId?: string;
  /**
   * For `type: 'teardown'`: `true` signals an INTENTIONAL teardown (tab close /
   * session end / app quit) → reap escaped detached tasks before disposing. A
   * transient disconnect / crash never sends this (just stdin-EOF) → no reap.
   * Only main can tell intentional from transient; inside here both are EOF.
   */
  reap?: boolean;
  /**
...
rl.on('line', (line) => {
  ...
  switch (msg.type) {
    ...
    case 'reload_skills':
      ...
      break;
    case 'teardown':
      // Intentional teardown intent from main. `reap:true` → reap escaped
      // detached tasks before disposing. We trigger shutdown() now (don't wait
      // for the follow-up stdin-close) so the reap definitely runs; the close
      // handler's shutdown() is idempotent via the `shuttingDown` guard.
      reapRequested = msg.reap === true;
      void shutdown();
      break;
    case 'clear_context': {
      if (msg.sessionId) {
...
// Teardown state. `reapRequested` is set true only by an intentional `teardown`
// message from main; a bare stdin-close (transient disconnect / crash) leaves it
// false → dispose without reaping. `shuttingDown` makes shutdown() idempotent
// (teardown message + stdin-close can both fire).
let reapRequested = false;
let shuttingDown = false;
// Bound the reap so a hung provider RPC can't stall exit. Main's force-kill grace
// (remote.ts) is set slightly longer so the process normally self-exits first.
  ...
  if (reapRequested) {
    try {
      const summary = await Promise.race([
        reapDetachedTasks(backends.values(), (level, msg) => serverLog(level, 'reaper', msg)),
        new Promise<null>((resolve) => setTimeout(() => resolve(null), REAP_TIMEOUT_MS)),
      ]);
      if (summary) {
        serverLog('info', 'reaper', `reaped ${summary.reaped}/${summary.enumerated} detached task(s) on teardown`);
      } else {
        serverLog('warn', 'reaper', `reap timed out after ${REAP_TIMEOUT_MS}ms — disposing anyway`);
      }
    } catch (err: any) {
      serverLog('error', 'reaper', `reap threw: ${err?.message ?? err}`);
    }
  }
  for (const b of backends.values()) b.dispose();
  process.exit(0);
