// Codex device-code login DRIVE (codex-specific, out-of-band from ACP).
//
// Spawns `codex app-server`, runs its JSON-RPC login handshake
// (initialize → account/login/start {chatgptDeviceCode} → await
// account/login/completed), and emits Shelf's device-login wire events. The
// transport is injectable so the drive logic is unit-tested without the binary;
// the real shape was confirmed live (T0.3a): account/login/start returns
