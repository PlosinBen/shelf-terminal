...
// Exercises the toolkit runtime path end-to-end (connection + session + drive)
// against the in-process mock agent — no stdio, no binary, no credentials.
describe('acp client drive (connection + session + turn)', () => {
  it('drives a prompt turn: streams chunks + finalizes a reply + tool card', async () => {
    ...
    const conn = openAcpConnection(mock, { name: 'test' });

    const wire: OutgoingMessage[] = [];
    const session = await startSession(conn.agent, { cwd: '/tmp/proj' });
    const stopReason = await drivePromptTurn(session, 'hi', (m) => wire.push(m));
    conn.close();
    ...
    expect(wire).toEqual([
      ...
      // finalize: accumulated streamed text upserts over the placeholder
  ...
  it('passes additionalDirectories to session/new', async () => {
    ...
    // Re-wrap to capture session/new params via a prompt (mock echoes only prompt);
    // instead assert the builder path does not throw and yields a session id.
    const conn = openAcpConnection(mock);
    const session = await startSession(conn.agent, { cwd: '/tmp/p', additionalDirectories: ['/tmp/p/codex'] });
