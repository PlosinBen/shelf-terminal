...
async function handleSend(msg: IncomingMessage) {
  ...
  // the turn spawns/uses the CLI, so session/new finds them and the CLI's config-home
...
rl.on('line', (line) => {
  ...
  switch (msg.type) {
    ...
    case 'reconnect': {
      ...
      // next caps/turn respawns and re-reads the config-home credentials the device-
